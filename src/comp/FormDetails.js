...
const FormDetails = () => {
    ...
    useEffect(() => {
        const fetchAccess = async () => {
            try {
                const response = await fetch(`http://localhost:5001/api/getUserAccess?formId=${id}&userId=${userId}`);
    ...
    return (
        <div>
            ...
            <div>
                  {form.user_id && form.user_id.toString() === userId && (
                    <Link to={`/answer/${form.id}`}>
                        <button style={{ marginRight: '10px' }}>{t("Ответить на форму")}</button>
                    </Link>
                )}
                {form.author_id && form.author_id.toString() === userId && (
    <Link to={`/EditForm/${form.id}`}>
        <Link to="/Responses">{t("Ответы на форму")}</Link>
