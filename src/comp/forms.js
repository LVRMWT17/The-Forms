...
const UserMenu = () => {
    ...
    useEffect(() => {
        const fetchForms = async () => {
            ...
            if (userId) {
                try {
                    const response = await fetch(`https://pond-catkin-supermarket.glitch.me/api/forms/${userId}`);
                    if (!response.ok) {
    ...
    return (
<div style={{ padding: '20px', border: '1px solid #ccc', borderRadius: '5px', width: '900px', textAlign: 'center', marginLeft: '300px'}}>
            ...
            {forms.length > 0 ? (
                ...
                <div>
                    <p style={{fontSize: '25px'}}>{t("У вас нет созданных форм")}</p>
                </div>
            )}
</div>            
    );
